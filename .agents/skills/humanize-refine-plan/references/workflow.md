...
Follow the original refinement phases below using the current Codex task model.
Use native file/search/edit/user-question tools. A convergence status assigned
during refinement records annotation resolution, not a new independent review.
Do not invent fresh reviewer agreement.
...
Resolve configuration by following the same precedence and merge semantics defined in `.agents/skills/humanize/scripts/lib/config-loader.sh`. Reuse that behavior; do not invent a separate refine-plan config model.

### Config Merge Semantics

Use the same layer order as `load_merged_config`:

1. Required default config: `.agents/skills/humanize/config/default_config.json`
...
```powershell
uv run .agents/skills/humanize/run-humanize.py scripts/validate-refine-plan-io.sh <validated-arguments>
```
...
Extract comments using a **stateful scanner** with the states and rules below, not a naive regular expression pass. If a helper script is needed, prefer `uv run script.py` (or native Python when uv is unavailable); never use WSL.
...
- In `discussion` mode: use the available user-question tool to confirm the classification before continuing
...
- `Codex-Codex Deliberation` (or the existing legacy `Claude-Codex Deliberation` heading) and `Pending User Decisions` reflect the new state
...
1. Perform targeted repository research using only read-only file, directory, and search tools.
...
- `## Dependencies and Sequence`
- `## Task Breakdown`
- `## Codex-Codex Deliberation` (accept and preserve legacy `## Claude-Codex Deliberation`)
...
Read `.agents/skills/humanize/prompt-template/plan/refine-plan-qa-template.md` and populate it completely. The QA document is not optional.

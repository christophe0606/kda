...
source "$SCRIPT_DIR/lib/python-runner.sh"
...
require_codex_hooks_support() {
    ...
    local features
    features=$(codex features list 2>/dev/null) || die "Could not read Codex features"
    if printf '%s\n' "$features" | grep -qE '^hooks[[:space:]]'; then
        CODEX_HOOKS_FEATURE=hooks
    elif printf '%s\n' "$features" | grep -qE '^codex_hooks[[:space:]]'; then
...
merge_hooks_json() {
    ...
    if ! humanize_python_available; then
        die "uv or Python is required to merge Codex hooks"
    fi

    humanize_run_python - "$hooks_file" "$template_file" "$runtime_root" <<'PY'
import json
import os
import pathlib
import re
        ...
        if isinstance(group, dict):
            for hook in group.get("hooks", []):
                if isinstance(hook, dict) and "command" in hook:
                    if os.name == "nt":
                        launcher_path = pathlib.Path(runtime_root) / "run-humanize.py"
                        launcher = runpy.run_path(str(launcher_path))
                        command = launcher["windows_hook_command"](runtime_root)
...
managed_pattern = re.compile(r"(^|[/\s])hooks/(loop-codex-stop-hook\.sh|pr-loop-stop-hook\.sh)(['\"\s]|$)")

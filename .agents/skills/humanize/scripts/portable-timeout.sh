...
#
# Priority: gtimeout (Homebrew) > timeout (GNU) > uv/Python > no timeout
#

source "$(cd "$(dirname "${BASH_SOURCE[0]}")/lib" && pwd)/python-runner.sh"
...
detect_timeout_impl() {
    ...
    elif humanize_python_available; then
        echo "python"
    else
        echo "none"
    fi

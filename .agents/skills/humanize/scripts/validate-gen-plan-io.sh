...
usage() {
    ...
    echo "  --discussion  Use discussion mode (independent planner/reviewer convergence rounds)"
...
MERGED_CONFIG=$(load_merged_config "$(cd "$SCRIPT_DIR/.." && pwd)" "$PWD")
GEN_PLAN_MODE=$(get_config_value "$MERGED_CONFIG" gen_plan_mode)
GEN_PLAN_MODE="${GEN_PLAN_MODE:-discussion}"
if [[ "$GEN_PLAN_MODE_DISCUSSION" == true ]]; then GEN_PLAN_MODE=discussion; fi
if [[ "$GEN_PLAN_MODE_DIRECT" == true ]]; then GEN_PLAN_MODE=direct; fi
if [[ "$GEN_PLAN_MODE" != discussion && "$GEN_PLAN_MODE" != direct ]]; then
    echo "ERROR: gen_plan_mode must be discussion or direct"
    exit 6
fi
echo "GEN_PLAN_MODE: $GEN_PLAN_MODE"
echo "CODEX_MODEL: $(get_config_value "$MERGED_CONFIG" codex_model)"
echo "CODEX_EFFORT: $(get_config_value "$MERGED_CONFIG" codex_effort)"
echo "ALTERNATIVE_PLAN_LANGUAGE: $(get_config_value "$MERGED_CONFIG" alternative_plan_language)"

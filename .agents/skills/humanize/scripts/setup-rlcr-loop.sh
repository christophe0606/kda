...
[[ "${HUMANIZE_PROVIDER_MODE:-}" == codex-only ]] && IMPLEMENTER_SESSION_ID="${CODEX_THREAD_ID:-}"
...
OPTIONS:
  ...
  --session-id <ID>     Bind a Codex loop to this session (defaults to CODEX_THREAD_ID)
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --require-bitlesson-entry-for-none)
            ...
            ;;
        --session-id)
            [[ -n "${2:-}" ]] || { echo 'Error: --session-id requires an ID' >&2; exit 1; }
            IMPLEMENTER_SESSION_ID="$2"
            shift 2
            ;;
        --privacy)
            PRIVACY_MODE="true"
...
if [[ -n "$IMPLEMENTER_SESSION_ID" && ! "$IMPLEMENTER_SESSION_ID" =~ ^[a-zA-Z0-9_-]+$ ]]; then
    echo 'Error: Invalid --session-id; use the actual Codex session ID.' >&2
    exit 1
fi
if [[ "${HUMANIZE_PROVIDER_MODE:-}" == codex-only ]]; then
    if [[ ! "$IMPLEMENTER_SESSION_ID" =~ ^[a-zA-Z0-9_-]+$ ]]; then
        echo 'Error: Start RLCR from the implementing Codex session (CODEX_THREAD_ID), or supply --session-id with that session ID.' >&2
        exit 1
    fi
    if [[ "$AGENT_TEAMS" == true ]]; then
        echo 'Error: --agent-teams is a Claude feature. Use Codex subagents from the implementing session instead.' >&2
        exit 1
    fi
fi

...
OPTIONS:
  --question-file <PATH>
                       Read the prompt from a UTF-8 file instead of command-line text
...
while [[ $# -gt 0 ]]; do
    ...
    case $1 in
        ...
        --question-file)
            if [[ -z "${2:-}" || "$2" == --* ]]; then
                echo "Error: --question-file requires a path" >&2
                exit 1
            fi
            QUESTION_FILE="$2"
            shift 2
...
if [[ -n "$QUESTION_FILE" ]]; then
    if [[ ${#QUESTION_PARTS[@]} -gt 0 || ! -f "$QUESTION_FILE" ]]; then
        echo "Error: --question-file requires an existing file and cannot be combined with question text" >&2
        exit 1
    fi

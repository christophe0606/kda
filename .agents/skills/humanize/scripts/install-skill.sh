...
install_codex_user_config() {
    ...
    if ! humanize_python_available; then
        die "uv or Python is required to update Humanize user config for Codex installs"
    fi
    ...
    humanize_run_python - "$default_config_file" "$user_config_file" "$install_target" <<'PY'
import json
import pathlib
import sys

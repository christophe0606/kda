...
source "$SCRIPT_DIR/../scripts/lib/python-runner.sh"
...
source "$PLUGIN_ROOT/scripts/lib/codex-cli.sh"
...
# Codex loops must be explicitly owned by the implementing session. Never let
# a legacy/unbound state file attach itself to an arbitrary project's session.
if [[ "${HUMANIZE_PROVIDER_MODE:-}" == codex-only ]]; then
    owner_state=$(resolve_active_state_file "$LOOP_DIR")
    owner_id=$(sed -n 's/^session_id: *//p' "$owner_state" | head -1 | tr -d '\r')
    if [[ -z "$HOOK_SESSION_ID" || -z "$owner_id" ]]; then
        jq -n '{decision:"block", reason:"This Codex loop has no session owner. Cancel and restart it from the implementing Codex session with CODEX_THREAD_ID or --session-id. Do not edit state.md manually."}'
        exit 0
    fi
    [[ "$owner_id" == "$HOOK_SESSION_ID" ]] || exit 0
fi
...
if [[ -f "$TODO_CHECKER" ]]; then
    ...
    TODO_INPUT="$HOOK_INPUT"
    if [[ "${HUMANIZE_PROVIDER_MODE:-}" == codex-only ]]; then
        TODO_INPUT=$(printf '%s' "$HOOK_INPUT" | jq '. + {humanize_host:"codex"}')
    fi
    TODO_RESULT=$(printf '%s' "$TODO_INPUT" | humanize_run_python "$TODO_CHECKER" 2>&1) || TODO_EXIT=$?
...
CODEX_EXEC_ARGS+=("${CODEX_AUTOMATION_ARGS[@]}" "-C" "$PROJECT_ROOT")

# Build Codex command arguments for codex review
CODEX_REVIEW_ARGS=("-c" "model=${CODEX_REVIEW_MODEL}" "-c" "review_model=${CODEX_REVIEW_MODEL}" "-c" 'approval_policy="never"' "-c" 'sandbox_mode="read-only"')
...
CODEX_EXIT_CODE=0
# A retried round must not reuse a previous malformed/partial review report.
# This path is generated from the validated loop directory and round number.

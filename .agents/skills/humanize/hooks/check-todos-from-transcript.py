...
def main():
    ...
    if hook_input.get("humanize_host") == "codex":
        # Codex uses update_plan function calls in its JSONL rollout, not
        # ~/.claude/tasks. A missing transcript is not evidence of completion;
        # the summary/goal-tracker and independent reviewer remain authoritative.
        ...
        if transcript and Path(transcript).is_file():
            with Path(transcript).open(encoding="utf-8") as stream:
                for line in stream:
                    try:
                        entry = json.loads(line)
                        item = entry.get("payload", {}) if entry.get("type") == "response_item" else entry
                        if item.get("type") != "function_call" or item.get("name") not in ("update_plan", "functions.update_plan"):
                            continue
                        arguments = item.get("arguments", {})
                        if isinstance(arguments, str):
                            arguments = json.loads(arguments)
                        if isinstance(arguments.get("plan"), list):
                            latest_plan = arguments["plan"]
        ...
        for step in latest_plan:
            ...
            if step.get("status") != "completed" and classify_lane(content) != "queued":
                incomplete_items.append({"status": step.get("status", "pending"), "content": content, "source": "plan"})
        if incomplete_items:

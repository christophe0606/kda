...
source "$(cd "$(dirname "${BASH_SOURCE[0]}")/../../scripts/lib" && pwd)/python-runner.sh"

# Native Windows git/realpath/Python can return drive-letter paths, whereas
# Bash's pwd returns /c/... . Normalize before containment checks or joins.
_humanize_shell_path() {
...
resolve_project_root() {
    ...
    _humanize_shell_path "${canonical:-$root}"
}
...
canonicalize_path_prefix() {
    ...
    if humanize_python_available; then
        parent_real=$(humanize_run_python -c 'import os,sys;print(os.path.realpath(sys.argv[1]))' "$parent" 2>/dev/null || true)
...
# If realpath is unavailable and uv/Python is missing, prints the input
# path verbatim.
#
...
canonicalize_path() {
    ...
    if canonical=$(realpath "$path" 2>/dev/null) && [[ -n "$canonical" ]]; then
        _humanize_shell_path "$canonical"
        return 0
    fi
    ...
    if canonical=$(realpath "$parent" 2>/dev/null) && [[ -n "$canonical" ]]; then
        _humanize_shell_path "${canonical%/}/$base"
        return 0
    fi

    if humanize_python_available; then
        canonical=$(humanize_run_python -c 'import os,sys;print(os.path.realpath(sys.argv[1]))' "$path" 2>/dev/null || true)
        if [[ -n "$canonical" ]]; then
            _humanize_shell_path "$canonical"
            return 0
        fi

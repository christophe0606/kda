...
block_methodology_analysis_incomplete() {
    ...
    if [[ "${HUMANIZE_PROVIDER_MODE:-}" == codex-only ]]; then
        jq -n --arg dir "$LOOP_DIR" '{decision:"block", reason:("Use this Codex session to complete the local process analysis. Write a nonempty report to " + $dir + "/methodology-analysis-report.md and a completion note to " + $dir + "/methodology-analysis-done.md. Do not change code or publish anything.")}'
        return
    fi

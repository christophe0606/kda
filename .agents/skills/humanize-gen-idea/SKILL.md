...
description: Generate a repository-grounded idea draft through exploration of alternative designs, without implementing it.
---
...
This command transforms a loose idea into a repo-grounded draft suitable as input to `$humanize-gen-plan`. It applies directed-diversity exploration: a lead picks N orthogonal directions, N read-only Codex subagent explorations develop each, the lead synthesizes a draft with one primary direction plus N-1 alternatives. Each direction carries objective evidence from the repo.
...
Do not interpret or rewrite the idea text here. Pass the parsed arguments to Phase 1 without rewriting their contents; keep the idea text as a single argument.
...
Run:
```powershell
uv run .agents/skills/humanize/run-humanize.py scripts/validate-gen-idea-io.sh '<idea text or input.md>' --n 6
# Pass the actual parsed --n and --output options when supplied.
```
...
- `file`: read the full contents of `IDEA_BODY_FILE` using available file-reading tools.
...
- Top-level directory listing using directory-listing tools (one level, no recursion).
...
Use Codex subagent tools to explore all directions, within the available concurrency limit. Queue excess directions in batches, reusing completed agents if necessary. Give each exploration only its assigned direction and the common input/context. Wait for all requested directions before synthesis; a concurrency limit does not reduce N.

### Subagent Invocation

For each direction in `DIRECTIONS`, run one read-only Codex subagent exploration, inheriting the current task model without a model override. Each invocation prompt MUST include:
...
Write the finalized content to `OUTPUT_FILE` using available file-writing tools. Single write; no progressive edits.

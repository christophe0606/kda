    ...
    const float32_t *pSrc, float32_t *pDst, uint32_t blockSize)
{
    __asm volatile(
        ...
        "cmp r3, #2\n"
        "beq 5f\n"
        /* R3 follows a full vector, so all four overlapping windows are
         * inside the input. Recompute the preceding output as well. */
        "vldrw.u32 q0, [r1, #-4]\n"
        "vmul.f32 q0, q0, r7\n"
        "vldrw.u32 q1, [r1, #-8]\n"
        "vfma.f32 q0, q1, r6\n"
        ...
        "pop {r4-r10, pc}\n"
        "4:\n"
        /* R1 follows a full vector. Reload its final four-input window and
         * exact coefficients directly into scalar-accessible vector lanes. */
        "vldrw.u32 q1, [r1, #-12]\n"
        "vldrw.u32 q2, [r0]\n"
        "vmul.f32 s0, s6, s10\n"
        "vfma.f32 s0, s5, s9\n"
        ...
        "pop {r4-r10, pc}\n"
        "5:\n"
        /* R2 keeps oldh0 and predicates both memory operations. */
        "mov r0, r8\n"
        "vctp.32 r3\n"
        "vpst\n"
        "vldrwt.u32 q1, [r1]\n"
